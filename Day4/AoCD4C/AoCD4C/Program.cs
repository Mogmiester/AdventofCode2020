...
namespace AoCD4C
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string InputFileName = "day4pw.txt";
            string fileAsString = File.ReadAllText(InputFileLocation + InputFileName);

            string listOfValidPassports = "";
            int countingPassports = 0;
            ...
            foreach (Dictionary<string, string> passportInfo in allPassportInfo)
            {
                ...
                if (acceptableID) {
                    listOfValidPassports += ", " + countingPassports.ToString();
                    acceptableDocuments++;
            ...
            File.WriteAllText(InputFileLocation + "outputforpw.txt", listOfValidPassports);
